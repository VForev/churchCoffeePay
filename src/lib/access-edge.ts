// Edge-runtime-safe helpers (no Node.js / supabase-js imports).
...
function getSigningKey(): string {
  ...
  if (!key || key.length < 16) throw new Error('SESSION_SIGNING_KEY is not configured');
  return key;
}
...
export function constantTimeKeyMatch(
...
export function isSessionActive(
  row: AccessSession | null | undefined,
  now = Date.now()
): boolean {
  if (!row) return false;
