...
manual step. **But the bundle has to be committed and pushed to reach the shop PC** — the
launcher downloads it from the GitHub raw URL, not from Netlify.

### "My label change didn't reach the printer"

Almost always the shop PC is running old code, not a layout bug. Check that first.

The zip carries a **version stamp** — `print-agent/VERSION.txt`, a short hash of the bundle's
contents, generated by `build-agent-zip.mjs`. Deliberately a content hash and not a
timestamp, so an unchanged build produces an unchanged zip instead of a git change every
time anyone runs `npm run dev`.

- `LOTG-Printer.bat` prints it on the shop PC when it starts.
- `/admin/print-setup` shows what it should be, read from `public/print-agent-version.txt`.
- Same letters = same code. Different = that PC didn't update; re-run the launcher.

The launcher also cache-busts its download (`?v=<random>`) and says so loudly when it falls
back to the copy already on the PC — a silently-cached zip is exactly how a machine ends up
running last week's code through any number of restarts.
